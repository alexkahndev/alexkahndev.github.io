...
const MuseumModel = ({ museumHoveredRef }) => {
  ...
  const springConfig = {
    hoverScale: hovered ? 1.15 : 1.0, // Adjusted to 1.05 for a slightly larger growth
  ...
  useEffect(() => {
    if (modelLoaded && museumRef.current && modelRef.current) {
      ...
      museumRef.current.position.set(100, 120, -100);
      museumRef.current.rotation.y = Math.PI * 2 + 0.5;
    }
  }, [modelLoaded]);
  ...
  const handleMouseEnter = () => {
      setHovered(true);
      museumHoveredRef.current = true;
  };
  
  const handleMouseLeave = () => {
      setHovered(false);
      museumHoveredRef.current = false;
  };
